...
def check_pro(obj,task_type,norms):
    ...
    #Return 1 if action vioaltes any rule
    for key,rule in norms.items():
        if rule[2][1]==task_type:
            if rule[1][1][1]==obj.colour:
                if rule[1][2][1]==obj.shape:
                    return (1,key)
                    break
    return (0,nan)
...
def check_per(obj,task_type,norms):
    from numpy import nan
    #Return 1 if action is permitted any rule
    for key,rule in norms.items():
        if rule[2][1]==task_type:
            if rule[1][1][1]==obj.colour:
                if rule[1][2][1]==obj.shape:
...
def verify_action(obj,task_type,rules):
    ...
    rule_dict={x:rules[x] for x in range(len(rules))}
